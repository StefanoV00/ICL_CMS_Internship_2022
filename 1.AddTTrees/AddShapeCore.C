...
void AddShapeCore () 
{
    ...
    const char* friend_name = (treename + "_ShapeCore").c_str();
    ...
    double Atot_pr;        tfriend->Branch("lc_Atot_pr", &Atot_pr);
    vector<double> Ag_pr(2); tfriend->Branch("lc_Ag_pr",   &Ag_pr); 

    double AtotBig_pr;       tfriend->Branch("lc_AtotBig_pr",&AtotBig_pr);
    vector<double> AgBig_pr(2);tfriend->Branch("lc_AgBig_pr",  &AgBig_pr);

    // Areas of projections (as convex hulls of all detections)
    double Astartot_pr;       tfriend->Branch("lc_Astartot_pr",&Astartot_pr);
    vector<double> Astarg_pr(2);tfriend->Branch("lc_Astarg_pr",  &Astarg_pr); 
    ...
    { 
        ...
        // First a lambda function to tell if it is core
        auto iscore = [BCl, deltacore] (int layer_n)
                        {return abs(layer_n - BCl)<=deltacore;};
        int N = x->size();
        ...
        vector <double> eneoverlapcore = getAwhereB(*eneoverlap,
                                                   *layerN, iscore);
        auto b = eneoverlapcore.begin(); auto e = eneoverlapcore.end();
        auto itmax = max_element(b, e);
        double Eomax = eneoverlapcore[distance(b, itmax)];
        ...
        double Eo2max = eneoverlapcore[distance(b, it2max)];

        vector <double> ene0  = getAwhereB(*energy, *which_g, 0);
        vector <int>    lay0  = getAwhereB(*layerN, *which_g, 0);
        ene0 = getAwhereB(ene0, lay0, iscore);
        ...
        vector <int>    lay1  = getAwhereB(*layerN, *which_g, 1);
        ene1 = getAwhereB(ene1, lay1, iscore);
        ...
        //cout<<"length: "<<last_all-first_all+1<<endl;
        //cout<<centrex->size()<<endl;
        //lc__________________________________________________________
        int firstcore = ceil (BCl - deltacore);
        int lastcore  = floor(BCl + deltacore);
        for (int ln=firstcore; ln<=lastcore; ln++) // for each layer
        {
            // maximum distances______________________________________
            vector <double> xn = getAwhereB(*x, *layerN, ln);
            vector <double> yn = getAwhereB(*y, *layerN, ln);
            //cout<<"# events: "<<xn.size()<<endl;
        ...
        vector <double> corexpr  = getAwhereB(*xpr,    *layerN, iscore);
        vector <double> coreypr  = getAwhereB(*ypr,    *layerN, iscore);
        vector <double> coreene  = getAwhereB(*energy, *layerN, iscore);
        vector <int> coreg    = getAwhereB(*which_g, *layerN, iscore);
        vector <int> corehits = getAwhereB(*nrechits,*layerN, iscore);
        ...
        vector <double> soln = get_max_distances(corexpr, coreypr, 
                                                 coreg,   corehits, 
                                                 cpvtx0_pr,  cpvtx1_pr,
        ...
        vector<double> radii =get_radii(corexpr, coreypr, 
                                    coreg, coreene,
        ...
        vector <double> xpr0 = getAwhereB(corexpr  , coreg, 0);
        vector <double> ypr0 = getAwhereB(coreypr  , coreg, 0);
        vector <double> xpr1 = getAwhereB(corexpr  , coreg, 1);
        vector <double> ypr1 = getAwhereB(coreypr  , coreg, 1);

        Atot_pr  = Shoelace(ConvexHull(corexpr, coreypr));
        Ag_pr[0] = Shoelace(ConvexHull(xpr0, ypr0));
        Ag_pr[1] = Shoelace(ConvexHull(xpr1, ypr1));

        auto shull = StarHull(corexpr, coreypr,
        ...
        vector <double> xprbig =getAwhereB(corexpr ,corehits, bighits);
        vector <double> yprbig =getAwhereB(coreypr ,corehits, bighits);
        vector <int>    gbig   =getAwhereB(coreg   ,corehits, bighits);
