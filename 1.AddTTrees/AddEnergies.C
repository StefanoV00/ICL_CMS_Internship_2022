...
void AddEnergies () 
{
    ...
    vector<int> length(2);tfriendS->Branch("ts_showerLength", &length);
    int whole_length = 0 ;tfriendS->Branch("ts_wholeLength",  &whole_length);
    
    // Effective Layer numbers, use lc info
    vector<int>layerg_start;tfriendS->Branch("lc_layerFromgStart",&layerg_start);
    ...
    vector <double> gmaxlcfraction (2);
    tfriend -> Branch("lc_gmaxLCFraction",    &gmaxlcfraction);
    vector <double> gmaxLfraction (2);
    tfriend -> Branch("lc_gmaxLayerFraction", &gmaxLfraction);
    ...
    vector <double> gBCx (2); 
    vector <double> gBCy (2); 
    vector <double> gBCz (2); 
    vector <double> gBCl (2); 
    vector <double> gBCzstart (2);
    vector <double> gBClstart (2);
    tfriend->Branch("lc_gBCx", &gBCx);
    tfriend->Branch("lc_gBCy", &gBCy);
    tfriend->Branch("lc_gBCz", &gBCz);
    tfriend->Branch("lc_gBCl", &gBCl);
    tfriend->Branch("lc_gBCzFromStart",&gBCzstart);
    tfriend->Branch("lc_gBClFromStart",&gBClstart);
    ...
    tfriend->Branch("lc_gBCx_pr", &gcentreX_pr);
    vector <double> gcentreY_pr(2);
    tfriend->Branch("lc_gBCy_pr", &gcentreY_pr);
    

    //Barycentre of layer, use lc
    ...
    //Barycentre of layer Tot Information
    ...
    //Barycentre Tot of layer from start
    ...
    { 
        ...
        if (i==icheck) {cout<<"Barycentre Tot works"<<endl;}

        // Individual Photons (plus some energy info for later)
        vector <double> lcEimax (2);
        vector <double> EiTot (2);
        for (int gi = 0; gi < 2; gi++)
        {
            vector <double> xi   = getAwhereB(*x     , *which_g, gi);
            vector <double> yi   = getAwhereB(*y     , *which_g, gi);
            vector <double> zi   = getAwhereB(*z     , *which_g, gi);
            vector <int>    li   = getAwhereB(*layerN, *which_g, gi);
            vector <double> xpri = getAwhereB(*xpr   , *which_g, gi);
            vector <double> ypri = getAwhereB(*ypr   , *which_g, gi);
            vector <double> enei = getAwhereB(*energy, *which_g, gi);

            gBCx[gi]        = mymean(xi,   enei);
            gBCy[gi]        = mymean(yi,   enei);
            gBCz[gi]        = mymean(zi,   enei);
            gBCl[gi]        = mymean(li,   enei);
            gBCzstart[gi]   = gBCz[gi] - zi[argmin(zi)];
            gBClstart[gi]   = gBCl[gi] - layerfirst->at(gi);
            gcentreX_pr[gi] = mymean(xpri, enei);
            gcentreY_pr[gi] = mymean(ypri, enei);

            lcEimax[gi] = enei[argmax(enei)];
            EiTot  [gi] = myvecsum(enei);
            gmaxlcfraction[gi] = lcEimax[gi] / EiTot[gi];
        }
        if (i==icheck) {cout<<"Barycentre of Prjections works"<<endl;}
        ...
        maxlcfraction = lcEmax / (EiTot[0] + EiTot[1]);
        ///*
        ...
        maxLfraction = LEmax / (EiTot[0] + EiTot[1]);
        gmaxLfraction[0] = Ed_ptr->at(0)[argmax(Ed_ptr->at(0))]/EiTot[0];
        gmaxLfraction[1] = Ed_ptr->at(1)[argmax(Ed_ptr->at(1))]/EiTot[1];
