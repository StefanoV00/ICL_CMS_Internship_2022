...
/// File to read information from a tree file to csv format.
/// Might occasionally crash: no big deal, just change values
/// in start variables (lines 72-76) to reprise from last saved.

/*
cd C:\root_v6.26.02\root-on-vscode 
C:\root_v6.26.02\bin\thisroot.bat 
cd 3.Analysis
root SaveAllInputs_alsonorm.C -q
cd ../
...
void SaveAllInputs_alsonorm (bool save_txt = true) 
{
////////////////////////////////////////////////////////////////////////
//////0. Set Variables   ///////////////////////////////////////////////
...
/////__________________________________________________________________
    ...
    for (int run = runstart; run < Nrun; run++)
    {
        ...
        if (dir)
        {
            ...
            tree->SetBranchAddress("lc_energy", &energy_all);
            tree->SetBranchAddress("lc_tsMult", &tsmult_all);
            vector <double>* x_all = 0;
            vector <double>* y_all = 0;
            vector <double>* z_all = 0;
            vector <int>* layer_all= 0;
            vector <int>* which_g  = 0;
            tree->SetBranchAddress("lc_x",     &x_all);
            tree->SetBranchAddress("lc_y",     &y_all);
            tree->SetBranchAddress("lc_z",     &z_all);
            tree->SetBranchAddress("lc_layer", &layer_all);
            ...
            { 
                ...
                vector<double> tsmultb=getAwhereB(*tsmult_all,*layer_all,
                                                               isecal);
                vector<double> xb     =getAwhereB(*x_all     ,*layer_all,
                                                               isecal);
                vector<double> yb     =getAwhereB(*y_all     ,*layer_all,
                                                               isecal);
                vector<double> zb     =getAwhereB(*z_all     ,*layer_all,
                                                               isecal);
                ...
                // Get Energies
                double Etot0 = 0; double Etot1 = 0;
                for (int j = 0; j<gb.size(); j++)
                {
                    if (gb[j]==0) Etot0 += energyb[j]/tsmultb[j];
                    else          Etot1 += energyb[j]/tsmultb[j];
                }
                double Etot = Etot0 + Etot1;
                if (Etot0 < Etot1) //Etot0 must be bigger
                {double a = Etot0*1.;Etot0 = Etot1*1.;Etot1 = a*1.;}

                // Keep only distinct lcs (do not repeat, ofc)
                vector<double> energy=getAwhereB(energyb, tsmultb,
                                                          distinct);
                vector<double> tsmult=getAwhereB(tsmultb, tsmultb,
                                                          distinct);
                vector<double> x     =getAwhereB(xb     , tsmultb,
                                                          distinct);
                vector<double> y     =getAwhereB(yb     , tsmultb,
                                                          distinct);
                vector<double> z     =getAwhereB(zb     , tsmultb,
                                                          distinct);
                vector<int>    layer =getAwhereB(layerb , tsmultb,
                                                          distinct);
                vector<int>    g     =getAwhereB(gb     , tsmultb,
                                                          distinct);
                for (int ij = g.size(); ij>0; ij--)
                {//perfect 2. (50-50) escape distinct lambda function
                    if (tsmult[ij] == 2. && g[ij] == 1)
                    {
                        energy.erase(energy.begin()+ij);
                        tsmult.erase(tsmult.begin()+ij);
                        x     .erase(x     .begin()+ij);
                        y     .erase(y     .begin()+ij);
                        z     .erase(z     .begin()+ij);
                        layer .erase(layer .begin()+ij);
                        g     .erase(g     .begin()+ij);
                    }
                }
                ...
                ////////////////////////////////////////////////////////
                // 3.2 TAKE USEFUL INFORMATION - Energy
                //______________________________________________________
                ...
                vector <double> Edepo;
                vector <int>    mult;
                for (int lay = 1; lay<=length; lay++)
                {
                    vector<double> energy_lay = getAwhereB(energy,layer, 
                                                                  lay);
                    Edepo.push_back(myvecsum(energy_lay));
                ...
                ////////////////////////////////////////////////////////
                // 3.3 TAKE USEFUL INFORMATION - PHOTON # 
                //______________________________________________________
                ...
                ////////////////////////////////////////////////////////
                // 3.4 TAKE USEFUL INFORMATION - PHOTON# (Projections)
                //______________________________________________________
                ...
                for (int j = 0; j<Nlcs; j++)
                    {if (esum[j] >= 0.68) 
                        ...
                        for (int jj = j; jj<Nlcs; jj++)
                            {if (esum[jj] >= 0.90) 
                                {rad90_pr = cdistances[jj];
                                for (int jjj = jj; jjj<Nlcs; jjj++)
                ...
                ////////////////////////////////////////////////////////
                // 3.5 TAKE USEFUL INFORMATION - BOTH
                //______________________________________________________
