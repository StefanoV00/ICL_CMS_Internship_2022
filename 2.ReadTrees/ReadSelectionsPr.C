...
//none if none
    ...
    double Atot_pr = 0;
    double AtotBig_pr = 0;
    trees->SetBranchAddress("lc_maxDistGCentre_pr", &maxDistGCen_pr);
    trees->SetBranchAddress("lc_maxDistCentre_pr",  &maxDistCen_pr);
    trees->SetBranchAddress("lc_maxDistSame_pr",    &maxDistG_pr);
    trees->SetBranchAddress("lc_maxDist_pr",        &maxDist_pr);
    trees->SetBranchAddress("lc_maxDistGCentre_pr", &maxDistGCen_pr);
    ...
    { 
        ...
        double csize=(c_prtot->GetWindowWidth()+c_prtot->GetWindowHeight())/2;
        c_prtot->SetWindowSize(csize, csize);
        TH2D* h_prtot    = new TH2D("Projections-All", "Projections-All", 
                                        100, xprmin, xprmax,
                                        100, yprmin, yprmax);
        ...
        TH2D* h_pr0    = new TH2D("Projection-0th photon", 
                                  "Projection-0th photon", 
                                        100, xprmin, xprmax,
                                        100, yprmin, yprmax);
        ...
        TH2D* h_pr1   = new TH2D("Projection-1st photon", "Projection-1st photon", 
                                        100, xprmin, xprmax,
                                        100, yprmin, yprmax);
        ...
        //auto pad = new TPad ();
        //pad -> SetTitle("Differentiating Photons");
        ...
        const char* textup = "Colour-Coded Projections";
        TText* text = new TText((xprmin+xprmax)/2,  yprmax+0.1, textup);
        text->SetTextAlign(21);
        text->SetTextSize(0.05);
        


        //h_projections -> FillN(Nentries, xpr, ypr, energy);
        for (int j = 0; j<N; j++)
        {
